...
<template>
    <div v-loading="loading" class="designer-preview">
        ...
        /> -->
        <report-preview-dialog ref="reportPreviewDialog" :preview-url="previewUrl" />
...
export default {
    ...
    components: {
        DataFilter,
        ReportPreviewDialog: () => import('./dialogMy.vue')
    },
    props: {
        previewUrl: {
            type: String,
    ...
    methods: {
        ...
        syncPreview() {
            ...
            this.loading = true
            let url
            if (this.paramsData.source === 'manager') {
                url = `/reportInfo/preview/actual/sync/${this.paramsData.reportId}`
            } else {
                url = `${this.previewUrl}/${this.paramsData.reportId}`
            }
