...
export default class DataProxy {
    ...
    pasteFromSystemClipboard(resetSheet, eventTrigger, event) {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.readText().then(content => {
                ...
                contentToPaste.forEach(row => {
                    ...
                    row.forEach(cellContent => {
                        this.setCellText(startRow, startColumn, cellContent, 'input', 0)
                        startColumn += 1
                    })
                    startRow += 1
                })
            ...
            contentToPaste.forEach(row => {
                let startColumn = selector.ci
                row.forEach(cellContent => {
                    const cell = rows.getCell(startRow, startColumn)
    ...
    // state: input | finished
    setCellText(ri, ci, text, state, type) {
        const { rows, history, validations } = this
        if (state === 'finished') {
            /* xreport start 1.修复历史记录BUG */
            // rows.setCellText(ri, ci, '')
            rows.setCellText(ri, ci, history.recordText(ri, ci))
            history.add(this.getData())
            rows.setCellText(ri, ci, text, '', '', '', type)
            history.recordText(ri, ci, text)
            /* xreport end */
        } else {
            rows.setCellText(ri, ci, text, '', '', '', type)
    ...
    updateLen() {
        ...
        const rowCells = Object.values(rows._).filter(row => row.cells).map(row => { return Math.max(...Object.keys(row.cells)) })
        if (rowCells.length) {
            const maxCellLength = Math.max(...rowCells)
            if (maxCellLength + 1 > cols.len) {
                cols.len = maxCellLength + 1
            }
        }
        const rowList = Object.keys(rows._)
        if (rowList.length) {
            const maxRowLength = Math.max(...rowList)
            if (maxRowLength + 1 > cols.len) {
                rows.len = maxRowLength + 1
            }
        }
    }
}
