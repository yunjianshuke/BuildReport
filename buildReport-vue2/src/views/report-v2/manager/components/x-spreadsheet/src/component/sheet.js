...
function overlayerMousescroll(evt) {
    ...
    const { deltaY, deltaX, shiftKey } = evt
    // 阻止默认滚动行为（关键：避免浏览器原生滚动冲突）
    evt.preventDefault()
    ...
    // 核心逻辑：根据Shift键判断滚动方向
    if (shiftKey) {
        // 按住Shift键：强制横向滚动（优先用deltaY转换，兼容多数鼠标）
        const horizontalDelta = deltaX || deltaY  // 鼠标滚轮通常用deltaY表示纵向，这里转为横向
        throttle(moveX(horizontalDelta), 50)
    } else {
        // 未按Shift键：正常处理纵向/横向滚动
...
/* xreport 复制bugfix */
function copy(evt) {
    const { data, selector, editor } = this
...
function overlayerMousedown(evt) {
    ...
    // console.log('selectorSetStart:::');
        ...
        /* xreport 13.销毁 */
        // mouse move up
        mouseMoveUp(window, this.onMouseMove = e => {
...
function dataSetCellText(text, state = 'finished') {
    ...
    const cell = data.getCell(ri, ci)
    if (data.settings.mode === 'read' || (cell && cell.editable === false)) return
...
function sheetInitEvents() {
    ...
    overlayerEl
        ...
        .on('mousedown', evt => {
            ...
            if (evt.buttons === 2) {
                ...
                if (this.data.settings.mode === 'preview') {
                    ...
                    if (cell.link) {
                        this.trigger('openLink', cell.link)
                    } else {
                        editorSet.call(this)
                    }
                } else {
        ...
        .on('mousewheel', evt => {
            ...
            evt.stopPropagation();
            if (evt.shiftKey) {
                // 横向滚动处理：deltaY为正向下滚动，对应横向向右滚动
                const scrollLeft = horizontalScrollbar.scroll().left + evt.deltaY;
                horizontalScrollbar.move({ left: scrollLeft });
            } else {
                // 正常纵向滚动处理
                overlayerMousescroll.call(this, evt);
            }
    ...
    // contextmenu
    contextMenu.itemClick = (type, num = 1) => {
    ...
    /* xreport 13.销毁 */
    bind(window, 'click', this.onClick)

    /* xreport 13.销毁 */
    bind(window, 'paste', this.onPaste)

    /* xreport 13.销毁 */
    bind(window, 'copy', this.onCopy)

    /* xreport 13.销毁 */
    // for selector
    bind(window, 'keydown', this.onKeyDown)
}

function createList() {
    this.trigger('createList')
}
...
export default class Sheet {
    constructor(targetEl, data) {
        ...
        // contextMenu
        this.contextMenu = new ContextMenu(() => this.getRect(), !showContextmenu, data.mode === 'preview')
        // selector
        ...
        /* xreport 公开sheetReset私有方法 */
        this.sheetReset = sheetReset
        // root element
        this.el.children(
            this.tableEl,
            this.overlayerEl.el,
    ...
    /* xreport 9.模拟Excel工具栏 */
    preview() {
        this.trigger('preview', this.data)
    }
  ...
  /* xreport 13.销毁 */
  onClick = evt => {
      this.focusing = this.overlayerEl ? this.overlayerEl.contains(evt.target) : false
  }
  ...
  onKeyDown = evt => {
      if (!this.focusing) return
      const { contextMenu, editor, toolbar } = this
  ...
  /* xreport 13.销毁 */
  sheetDestoryEvents() {
      unbind(window, 'resize', this.onResize)

      unbind(window, 'click', this.onClick)

      unbind(window, 'paste', this.onPaste)

      unbind(window, 'copy', this.onCopy)

      // for selector
      unbind(window, 'keydown', this.onKeyDown)

      unbind(window, 'mousemove', this.onMouseMove)
      if (window.xEvtUp) {
          unbind(window, 'mouseup', window.xEvtUp)
          window.xEvtUp = undefined
      }
  }

  /* xreport 13.销毁 */
  destroy() {
      this.sheetDestoryEvents()
  }
